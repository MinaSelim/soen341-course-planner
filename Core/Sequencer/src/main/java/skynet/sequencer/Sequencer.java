...
//			System.out.println(requiredCourse.getCourseCode() + ": " + requiredCourse.getPriority());
		...
		/*
		 * Step 3: Generate the sequence. This is achieved by iterating over the
		 * required list of courses until it is depleted. The available list contains
		 * courses from the required list that currently have all of their prerequisites
		 * satisfied and can be added to a semester's courses[].
		 */
		int year = 2019;
		int season = 0;
		...
		while (!required.isEmpty() || !available.isEmpty()) {
				...
				if (courseOffered) {
					if (course.hasCoreqs()) // the course is offered and has coreqs
					{
						...
						for (int index = 0; index < corequisites.length; index++) {
							...
							if (!contains((Course) corequisite, taken)
									&& !contains((Course) corequisite, registeredCourses)) {
								coreqsMet = false;
			...
			if (season >= 18) // Assuming six year limit with three seasons per year
			{
				throw new SequencerException("Sequencer Error: Check Input Courses");
			}
		}
